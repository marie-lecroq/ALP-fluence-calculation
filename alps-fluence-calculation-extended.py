import numpy as np
import scipy as sc
import scipy.integrate as integ
from scipy.interpolate import interp1d
from math import *
import numpy.random as rd
import pickle
#from tqdm import tqdm

from mpi4py import MPI

alpha = 1.0/137.035999084 #fine structure constant
d = 51.4*3.08567758149137e19 #distance from SN1987A to the Earth
Reff = 3.0e10 #effective radius of the SN below which photons are not released
c = 3.0e8

dt = 223.0 #duration of the measurement
ks = 16.8e6 #effective Debye screening scale in eV
C = 2.54e71 #multiplicative constant in eV-1
T = 30.6e6 #effective temperature in eV

min_erg, max_erg = 0.0, 5.0e8
erg_window_lo, erg_window_up = 2.5e7, 1.0e8

# Beta factor
def betafactor(m,E):
    return np.sqrt(1.0 - m*m/(E*E))

# Inverse of cot(x) = cos(x)/sin(x)
def arccot(x):
    res = 0.5*np.pi - np.arctan(x)
    return res

# Fluence for one(!) photon
def F_naive(m,g):
    res = integ.quad(lambda E : spectrum(m,g,E), min_erg, max_erg)[0]
    return (1.0/(4.0*np.pi*d*d)) * res / 1.0e4


# Inverse CDF of angular distribution
def angDistInvCDF(u,b):
    x = 2.0*u - 1.0
    return (b + x)/(1.0 + b*x)

#typical decay length according to eq(3) in arXiv:1702.02964
def l_ALP(m,g,E):
    x = 1.0e7/m
    x2 = x*x
    y = 1.0e-19/g
    return 4.0e13 * betafactor(m,E) * (E/1.0e8) * (x2*x2) * (y*y)

# Mass-dependent cross section
def sigma(m,g,E):
    res = 0.0
    if E > m:
        prefactor = 0.125*alpha*g*g
        if (m < 1.0e5):
            x = 0.25*ks*ks/(E*E)
            res = prefactor * ( (1.0 + x)*np.log(1.0 + 1.0/x) - 1.0 )
        else:
            e2 = E*E
            m2 = m*m
            m4 = m2*m2
            k2 = ks*ks
            bf = betafactor(m,E)
            yp = 2.0*e2*(1.0 + bf) - m2
            ym = 2.0*e2*(1.0 - bf) - m2
            res = prefactor * ( (1.0 + 0.25*k2/e2 - 0.5*m2/e2)*np.log((yp + k2)/(ym + k2)) - bf - (0.25*m4/(k2*e2))*np.log((m4 + k2*yp)/(m4 + k2*ym)) )
    return res

#energy distribution for the ALPs
def spectrum(m,g,E):
    res = 0.0
    if E > 0:
        res = C*E*E*sigma(m,g,E)/(np.exp(E/T) - 1.0)
    return res

number_alps = int(1e7) #number of particles of fixed m and g in the simulation ; according to arXiv:1702.02964, results are stable for number_alps > 1e7 ; it is the default value

erg_vals = np.linspace(min_erg, max_erg, num=5001)
cdf_vals = []
# It is okay to fix the value of g as it just rescales the spectrum
norm = integ.quad(lambda E : spectrum(0.0,1.0e-20,E), min_erg, max_erg)[0]
for erg in erg_vals:
    temp = integ.quad(lambda E : spectrum(0.0,1.0e-20,E), min_erg, erg)[0]
    if norm > 0 :
        cdf_vals.append(temp/norm)
    else :
        cdf_vals.append(0.0)
inv_cdf_massless = interp1d(cdf_vals, erg_vals, kind='linear')

#theoretical measured fluence
def F_exp_new(m,g):
    naive_fluence = F_naive(m,g)
    inv_cdf = inv_cdf_massless
    # Only recalculate CDF from spectrum for heavier axions; otherwise the m = 0 version is used.
    if (m > 1.0e6):
        cdf_vals = []
        new_max_erg = min(500.0*m,1.5e9)
        n = int((new_max_erg-m)/(0.1*m))+1
        erg_vals = np.linspace(m, new_max_erg, num=n)
        print(n)
        # It is okay to fix the value of g as it just rescales the spectrum
        norm = integ.quad(lambda E : spectrum(m,1.0e-20,E), m, new_max_erg)[0]
        for erg in erg_vals:
            temp = integ.quad(lambda E : spectrum(m,1.0e-20,E), m, erg)[0]
            cdf_vals.append(temp/norm)
        inv_cdf = interp1d(cdf_vals, erg_vals, kind='linear')
        print(cdf_vals)

    counts = 0


    # Generate random numbers from [0,1]
    rng_ergs = rd.random_sample(number_alps)
    rng_angles = rd.random_sample(number_alps)

    # Draw random energy values via inverse cdf
    rng_ergs = inv_cdf(rng_ergs)

    for i in range(number_alps):
        E = rng_ergs[i]
        if (E > m):
            l = l_ALP(m,g,E)
            L1 = rd.exponential(scale=l, size=None)
            if (L1 < d) and (L1 > Reff):
               bf = betafactor(m,E)
               # Draw a random angle in the axion rest frame, cos(angle), from [-1,1] (i.e. angle in [0,pi])
               phi = 2.0*np.pi*rd.random_sample()
               cos_theta = 2.0*rd.random_sample() - 1.0
               sin_theta = np.sin(np.arccos(cos_theta))
               cos_phi = np.cos(phi)
               sin_phi = np.sin(phi)
               x = (1.0 - bf*bf)*(sin_phi*sin_phi*sin_theta*sin_theta + cos_theta*cos_theta)
               # Photon 1 -> transform angle, energy, and L1 to lab frame
               E1 = 0.5*E*(1.0 + bf*sin_theta*cos_phi)
               if (E1 > erg_window_lo) and (E1 < erg_window_up):
                   angle1 = bf + cos_phi*sin_theta
                   angle1 = np.arccos(angle1/np.sqrt(x + angle1*angle1))
                   y1 =  L1*np.sin(angle1)
                   L21 = -L1*np.cos(angle1) + np.sqrt(d*d - y1*y1)
                   time1 = (L1/bf + L21 - d)/c
                   if (time1 < dt) and (time1 > 0):
                       counts += 1
               # Photon 2 -> transform angle, energy, and L1 to lab frame
               E2 = 0.5*E*(1.0 - bf*sin_theta*cos_phi)
               if (E2 > erg_window_lo) and (E2 < erg_window_up):
                   angle2 = bf - cos_phi*sin_theta
                   angle2 = np.arccos(angle2/np.sqrt(x + angle2*angle2))
                   y2 =  L1*np.sin(angle2)
                   L22 = -L1*np.cos(angle2) + np.sqrt(d*d - y2*y2)
                   time2 = (L1/bf + L22 - d)/c
                   if (time2 < dt) and (time2 > 0):
                        counts += 1
    res = open('results_new.txt','a')
    res.write(' '.join((str(np.log10(m)), str(np.log10(g)), str(counts/float(number_alps) * naive_fluence))) + '\n')
    res.close()



comm = MPI.COMM_WORLD
rank = comm.Get_rank()


cores = comm.Get_size() #number of cores you want to use

logm=np.arange(3,9,0.05)
logg=np.arange(-13,-3,0.05) #in Gev

couples = np.zeros((len(logm)*len(logg),2))

indice = 0
for i in logm:
    for j in logg:
        couples[indice]=[i,j]
        indice += 1

print(couples)


numPoints = len(logm)*len(logg)

numProcess = numPoints//cores

for i in range(numProcess):
    for j in range(cores):
        if rank  == j:
            print(F_exp_new(10**couples[i*cores+j,0],10**(couples[i*cores+j,1] -9))) #g in eV

leftover = numPoints - numProcess * cores

for k in range (leftover) :
    if rank == k:
        print(F_exp_new(10**couples[numProcess*cores + k,0],10**(couples[numProcess*cores + k,1] -9)))


